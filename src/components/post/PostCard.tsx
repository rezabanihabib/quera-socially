...
import { Heart, ImageOff, MessageCircle, Pencil, Trash2 } from "lucide-react";
...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    <article className="rounded-xl border border-border bg-surface p-4 shadow-card transition-colors">
      ...
      {post.image && (
        imageFailed ? (
          <div className="mt-3 flex h-40 w-full flex-col items-center justify-center gap-1.5 rounded-lg bg-muted text-muted-foreground">
      ...
      {editOpen && (
        <EditPostModal open={editOpen} onClose={() => setEditOpen(false)} post={post} />
      )}
    </article>
  );
}
